
import { Story, QuestionCategory } from '../../types';

export const elGuardianDeLasLucesPerdidas: Story = {
  id: 'el-guardian-de-las-luces-perdidas',
  title: 'El Guardián de las Luces Perdidas: La Gran Búsqueda de la Luz Eterna',
  author: 'Crónicas de Dream Land',
  coverImage: 'https://picsum.photos/seed/kirby-light/600/400',
  content: `
En las colinas verdes y soleadas de Dream Land, donde las flores cantan con el viento y las nubes parecen algodón de azúcar, vivía Kirby. Era una bolita rosa llena de curiosidad, con ojos grandes y brillantes que siempre buscaban una nueva aventura. Kirby no necesitaba palabras para ser un héroe; sus acciones hablaban por él. Pasaba los días flotando por el cielo, comiendo maxim tomates jugosos que crecían en árboles gigantes, y jugando con sus amigos, como Bandana Waddle Dee, un valiente Waddle Dee con una bandana roja y una lanza afilada que usaba para saltar alto o defender a los suyos.

Un día, mientras Kirby rodaba por una pradera comiendo una manzana del tamaño de una pelota de fútbol, el cielo cambió de golpe. No fue una tormenta normal: las nubes se volvieron grises como humo, y el sol pareció esconderse detrás de una cortina invisible. De pronto, cayó una lluvia extraña, no de agua, sino de gotitas brillantes que se apagaban al tocar el suelo, como estrellas fugaces muriendo.

Bandana Waddle Dee llegó corriendo, jadeando y con su lanza temblando en la mano. “¡Kirby! ¡Algo terrible está pasando! ¡Las luces de Dream Land se están yendo! Mira a los Waddle Dees: ¡están perdiendo su brillo!”

Kirby miró alrededor y vio que era verdad. Los Waddle Dees, que normalmente corrían y saltaban con ojos chispeantes, ahora se movían lentos como tortugas cansadas, con las pupilas opacas como ventanas empañadas. Las flores perdían sus colores vibrantes, volviéndose grises y marchitas. Hasta el río que brillaba como un espejo ahora parecía un charco de tinta. Algo o alguien estaba robando la luz del mundo entero.

Kirby no dudó. Infló sus mejillas como un globo rosa y se elevó hacia el cielo nublado. Bandana lo siguió, usando su lanza como un bastón para impulsarse. En lo alto, vieron un portal extraño: no era negro como los vórtices de antes, sino plateado y transparente, como un espejo roto flotando en el aire. Dentro se veían islas de cristal suspendidas en un vacío estrellado, pero las estrellas estaban apagadas, como bombillas fundidas.

“¡Vamos, Kirby! ¡Tenemos que averiguar qué pasa!”, gritó Bandana. Kirby hizo un sonido como “Poyo!” y cruzaron el portal juntos.

Al otro lado, cayeron suavemente en un mundo mágico pero triste: la Tierra de las Luces Perdidas. Era un reino flotante de islas hechas de cristal puro, conectadas por puentes de luz tenue. Cada isla tenía una lámpara gigante en el centro, como faros antiguos, pero la mayoría estaban apagadas y cubiertas de polvo cósmico. El aire era fresco y silencioso, roto solo por un zumbido bajo, como el de una bombilla a punto de quemarse. En el vacío alrededor, estrellas muertas flotaban como fantasmas.

En la isla central, la más grande de todas, flotaba una figura alta y esbelta: Luminara, una criatura hecha de cristal quebrado, con alas que parecían vidrios rotos y un pecho que brillaba débilmente con una esfera oscura en el centro. Esa esfera absorbía cualquier rayito de luz que se acercara, como un agujero negro hambriento. Sus ojos eran dos pozos vacíos, sin emoción.

“Bienvenidos a mi reino apagado”, dijo con una voz que crujía como pasos en vidrio roto. “La luz es finita, como un tesoro que se acaba. Yo la necesito toda para sobrevivir. Vuestro mundo será el siguiente en oscurecerse.”

Kirby frunció el ceño (o lo que sea que hace una bolita rosa para fruncir el ceño) y se preparó. Inhaló una ráfaga de viento brillante que flotaba cerca de una lámpara medio apagada y se transformó en Spark Kirby, con chispas eléctricas bailando alrededor de su cuerpo. Lanzó rayos que rebotaron por las islas de cristal, iluminando todo como un espectáculo de fuegos artificiales. Pero Luminara levantó una mano de cristal y creó un escudo de oscuridad que absorbió los rayos como si fueran gotas de lluvia en un desierto.

“¡No puedes vencerme con copias de luz robada!”, rugió Luminara. “¡Yo soy la ausencia misma de la luz! ¡La devoradora eterna!”

La batalla estalló como un trueno. Las islas de cristal comenzaron a girar lentamente, como un móvil gigante colgado del cielo. Luminara lanzaba fragmentos afilados de sus alas, que volaban como flechas negras. Kirby flotaba y esquiva, usando su forma spark para contraatacar con bolas de electricidad que explotaban al impacto. Bandana Waddle Dee no se quedaba atrás: giraba su lanza como un torbellino, rompiendo los fragmentos en el aire y protegiendo a Kirby de ataques sorpresa.

Pero cada vez que un fragmento tocaba el suelo de cristal, nacía un Sombrasol, una criatura oscura con forma de estrella puntiaguda que se arrastraba hacia ellos, intentando apagar su brillo con tentáculos de sombra. “¡Kirby, son demasiados!”, gritó Bandana mientras lanzaba su lanza contra uno, haciendo que se disolviera en humo negro.

Kirby cambió de táctica. Inhaló a un Sombrasol entero y se transformó en Dark Kirby, una versión suya con sombras rosadas que brillaban desde adentro, como una linterna en la noche. Con este poder, podía atravesar los escudos de oscuridad de Luminara y golpearla directamente en su esfera negra. ¡Pum! Un golpe hizo que el cristal de Luminara crujiera, y un pedacito de luz escapó de su pecho.

Luminara se enfureció. Con un aleteo de sus alas rotas, creó un Espejo de la Noche, un portal reflectante que devolvía todos los ataques. Un rayo de Kirby rebotó y casi golpea a Bandana, quien rodó a un lado justo a tiempo. “¡Cuidado! ¡Sus trucos son sucios!”, advirtió el Waddle Dee.

La pelea se volvió más intensa. Luminara flotaba entre las islas, teletransportándose como un fantasma, lanzando olas de oscuridad que apagaban las pocas lámparas que quedaban. Kirby y Bandana saltaban de isla en isla, usando los puentes de luz como rampas. En una isla pequeña, Kirby encontró un poder nuevo: inhaló un cristal brillante y se volvió Crystal Kirby, con un cuerpo transparente que reflejaba ataques. Así, devolvió una ola de oscuridad a Luminara, quien gritó de dolor.

Pero estaban cansados. “¡No podemos seguir así solos!”, jadeó Bandana. Justo entonces, de una de las últimas lámparas que titilaba débilmente, surgió un resplandor suave. De él se formó una figura pequeña y delicada: Luziel, un sercito hecho de luz pura, con forma de libélula y ojos como luciérnagas vivas. Era la última guardiana de las luces perdidas, escondida para sobrevivir.

“¡Deténganse!”, suplicó Luziel con una voz como campanitas. “Luminara no siempre fue así. Éramos amigas, las más brillantes de este mundo. Jugábamos a encender estrellas y a pintar auroras en el cielo. Pero un día, Luminara encontró la esfera negra, un antiguo artefacto de oscuridad que prometía luz infinita. La tentó: ‘Guarda la luz para ti, no la compartas’. Y se rompió por dentro, convirtiéndose en esto.”

Luziel flotó hacia Kirby y le tocó la mejilla con una alita cálida. “Toma mi luz, héroe rosa. Úsala no para destruir, sino para sanar.”

Kirby, con cuidado, inhaló la esencia de Luziel. No fue un poder destructivo: se transformó en Light Kirby, una versión luminosa de sí mismo, con alas de estrella rosadas y un aura que iluminaba todo el reino como un amanecer. Su cuerpo brillaba con colores suaves, y donde toca, las sombras retrocedían.

Ahora, la batalla final fue legendaria. Las islas giraban como un torbellino, el vacío estrellado rugía con vientos cósmicos. Luminara invocó su ataque supremo: un Vórtice de la Noche Eterna, un remolino negro que succionaba toda la luz restante, amenanzando con apagar no solo su mundo, sino Dream Land también. Los Sombrasoles se unieron, formando una pared de oscuridad.

Kirby flotó directo al centro, su aura light protegiéndolo. Bandana y Luziel lo ayudaban: el Waddle Dee rompía los espejos con su lanza, y Luziel enviaba destellos guía para mostrar debilidades. “¡Allí, en su pecho!”, gritaba Luziel.

En el clímax, cuando el vórtice estaba a punto de tragárselos, Kirby hizo lo impensable. En vez de lanzar un ataque final… abrazó a Luminara. Voló directo a su pecho, envolviendo la esfera negra con sus bracitos rosados llenos de luz pura. La esfera crujió como hielo rompiéndose. Se oyó un sonido desgarrador, como vidrio quebrándose desde el interior.

“¡No! ¡La luz es mía sola! ¡No la compartiré!”, rugió Luminara, intentando apartarlo.

Pero Kirby no soltó. Su luz no quemaba como un fuego destructivo; calentaba como el sol de la mañana, recordando a Luminara lo que era compartir. La esfera negra se agrietó más y más. De las grietas brotó luz robada: ríos de brillo que volvieron a las lámparas apagadas, encendiéndolas una a una. Las islas de cristal brillaron como diamantes, las estrellas muertas revivieron en explosiones de color, y hasta Dream Land, al otro lado del portal, recuperó su sol radiante.

Luminara cayó de rodillas en la isla central. Sus alas rotas comenzaron a recomponerse, pero ahora eran transparentes y llenas de arcoíris danzantes. Sus ojos vacíos se llenaron de lágrimas de cristal que brillaban como gotas de rocío.

“¿Por qué… no me destruiste? ¿Por qué me diste tu luz?”, preguntó con voz temblorosa.

Kirby simplemente sonrió y le dio una palmadita suave en la cabeza, como diciendo: “Porque todos merecen brillar”.

Luziel flotó hasta su amiga recuperada. “La luz no se guarda en una esfera oscura… se comparte como un regalo. Así dura para siempre”.

Luminara miró alrededor: las islas flotaban en armonía, las lámparas ardían con fuerza, y el vacío ahora era un mar de estrellas vivas. Se levantó lentamente, extendiendo sus alas nuevas. “Yo… olvidé cómo era ser luz para los demás. Gracias, pequeño héroe rosa”.

Juntos, Luminara, Luziel, Kirby y Bandana caminaron hasta la lámpara más grande del reino. Luminara puso sus manos en ella y dejó salir toda la luz que había acumulado por egoísmo. El cielo se llenó de auroras boreales, y un puente de luz eterna conectó la Tierra de las Luces Perdidas con Dream Land para siempre.

Cuando volvieron a casa, los Waddle Dees bailaban con ojos chispeantes, las flores explotaban en colores nuevos, y hasta el rey Dedede, desde su castillo, soltó una risita al ver el sol más brillante que nunca. Meta Knight, oculto en las sombras, inclinó la cabeza en silencio, reconociendo el coraje de Kirby.

Y Kirby, como siempre, se acostó en su nube favorita, mordisqueando una manzana fresca, feliz de que el mundo volviera a brillar con luz compartida.
    `,
  wordCount: 1184,
  level: '4to Grado (Nivel Q)',
  questions: {
    [QuestionCategory.LITERAL]: [
      {
        id: 'lp-lit1',
        category: QuestionCategory.LITERAL,
        questionText: '¿Qué le pasaba a los Waddle Dees cuando perdían su luz?',
        options: ['Se volvían gigantes', 'Se movían lentos como tortugas cansadas', 'Se convertían en mariposas', 'Se ponían a dormir en el suelo'],
        correctAnswer: 'Se movían lentos como tortugas cansadas',
      },
      {
        id: 'lp-lit2',
        category: QuestionCategory.LITERAL,
        questionText: '¿De qué material estaban hechas las islas de la Tierra de las Luces Perdidas?',
        options: ['De madera', 'De algodón de azúcar', 'De cristal puro', 'De metal oxidado'],
        correctAnswer: 'De cristal puro',
      },
      {
        id: 'lp-lit3',
        category: QuestionCategory.LITERAL,
        questionText: '¿Cuál fue la última transformación de Kirby durante la batalla final?',
        options: ['Spark Kirby', 'Dark Kirby', 'Crystal Kirby', 'Light Kirby'],
        correctAnswer: 'Light Kirby',
      },
    ],
    [QuestionCategory.SEQUENCING]: [
      {
        id: 'lp-seq1',
        category: QuestionCategory.SEQUENCING,
        questionText: '¿Qué pasó justo después de que Kirby inhalara la esencia de Luziel?',
        options: ['Luminara escapó', 'Se transformó en Light Kirby con alas de estrella', 'Bandana Waddle Dee atacó con su lanza', 'El portal se cerró'],
        correctAnswer: 'Se transformó en Light Kirby con alas de estrella',
      },
      {
        id: 'lp-seq2',
        category: QuestionCategory.SEQUENCING,
        questionText: '¿Qué hicieron Kirby y Bandana justo después de cruzar el portal de espejo?',
        options: ['Se pelearon con un dragón', 'Cayeron suavemente en la Tierra de las Luces Perdidas', 'Comieron una manzana gigante', 'Rescataron a un Waddle Dee'],
        correctAnswer: 'Cayeron suavemente en la Tierra de las Luces Perdidas',
      },
      {
        id: 'lp-seq3',
        category: QuestionCategory.SEQUENCING,
        questionText: '¿Qué sucedió inmediatamente después de que la esfera negra se rompiera?',
        options: ['Luminara se volvió más mala', 'Ríos de brillo volvieron a las lámparas y estrellas', 'Kirby se fue a dormir', 'Luziel desapareció'],
        correctAnswer: 'Ríos de brillo volvieron a las lámparas y estrellas',
      },
    ],
    [QuestionCategory.VOCABULARY]: [
      {
        id: 'lp-voc1',
        category: QuestionCategory.VOCABULARY,
        questionText: 'Luminara tenía un pecho que brillaba "débilmente". ¿Qué significa débilmente?',
        options: ['Con muchísima fuerza', 'Con un color muy fuerte', 'Con poca fuerza o intensidad', 'Que no brilla nada'],
        correctAnswer: 'Con poca fuerza o intensidad',
      },
      {
        id: 'lp-voc2',
        category: QuestionCategory.VOCABULARY,
        questionText: 'La batalla fue "legendaria". ¿Qué significa legendaria en este cuento?',
        options: ['Que duró muy poco', 'Muy importante, famosa y extraordinaria', 'Que pasó hace dos minutos', 'Que fue aburrida'],
        correctAnswer: 'Muy importante, famosa y extraordinaria',
      },
      {
        id: 'lp-voc3',
        category: QuestionCategory.VOCABULARY,
        questionText: 'Luziel era un ser hecho de luz "pura". ¿Qué significa pura?',
        options: ['Sucia', 'Mezclada con sombras', 'Limpia, sin mezclas, auténtica', 'De muchos colores oscuros'],
        correctAnswer: 'Limpia, sin mezclas, auténtica',
      },
    ],
    [QuestionCategory.MAIN_IDEA]: [
      {
        id: 'lp-main1',
        category: QuestionCategory.MAIN_IDEA,
        questionText: '¿Cuál es la enseñanza o moraleja principal de esta historia?',
        options: ['Que los portales son peligrosos', 'Que la luz dura más y es más poderosa cuando se comparte con los demás', 'Que hay que comer muchos tomates', 'Que las lanzas son mejores que las espadas'],
        correctAnswer: 'Que la luz dura más y es más poderosa cuando se comparte con los demás',
      },
      {
        id: 'lp-main2',
        category: QuestionCategory.MAIN_IDEA,
        questionText: '¿Por qué Luminara se volvió un monstruo de cristal quebrado al principio?',
        options: ['Porque tenía frío', 'Porque la tentó la idea de guardar la luz solo para ella', 'Porque Kirby la atacó', 'Porque se le rompieron las alas'],
        correctAnswer: 'Porque la tentó la idea de guardar la luz solo para ella',
      },
    ],
    [QuestionCategory.INFERENCE]: [
      {
        id: 'lp-inf1',
        category: QuestionCategory.INFERENCE,
        questionText: '¿Por qué el abrazo de Kirby fue más efectivo que sus rayos eléctricos contra Luminara?',
        options: ['Porque Kirby era más grande', 'Porque el amor y la calidez de compartir tocaron el corazón de Luminara y rompieron su egoísmo', 'Porque Kirby tenía mucha fuerza en los brazos', 'Porque Luminara se asustó'],
        correctAnswer: 'Porque el amor y la calidez de compartir tocaron el corazón de Luminara y rompieron su egoísmo',
      },
      {
        id: 'lp-inf2',
        category: QuestionCategory.INFERENCE,
        questionText: '¿Por qué los Waddle Dees perdían su brillo y alegría cuando Luminara robaba la luz?',
        options: ['Porque tenían sueño', 'Porque la luz representa la energía y la felicidad de Dream Land', 'Porque no les gustaba el color gris', 'Porque querían manzanas'],
        correctAnswer: 'Porque la luz representa la energía y la felicidad de Dream Land',
      },
    ],
    [QuestionCategory.CHARACTER]: [
      {
        id: 'lp-char1',
        category: QuestionCategory.CHARACTER,
        questionText: '¿Cómo describirías a Luziel?',
        options: ['Malvada y envidiosa', 'Amable, compasiva y dispuesta a ayudar', 'Miedosa y callada', 'Fuerte pero egoísta'],
        correctAnswer: 'Amable, compasiva y dispuesta a ayudar',
      },
      {
        id: 'lp-char2',
        category: QuestionCategory.CHARACTER,
        questionText: '¿Qué cualidad de Kirby se destaca más al final de la historia?',
        options: ['Su gran apetito', 'Su capacidad de perdonar y mostrar bondad incluso a un enemigo', 'Su velocidad para correr', 'Su habilidad con la lanza'],
        correctAnswer: 'Su capacidad de perdonar y mostrar bondad incluso a un enemigo',
      },
    ],
  },
};


import { Story, QuestionCategory } from '../../types';

export const laLeyendaDeFectoElfilis: Story = {
  id: 'la-leyenda-de-fecto-elfilis',
  title: 'La Leyenda de Fecto Elfilis: Del Caos Estelar a la Luz Eterna',
  author: 'Crónicas de Dream Land',
  coverImage: 'https://picsum.photos/seed/fecto-elfilis/600/400',
  content: `
Hace eones, en las profundidades del universo donde las estrellas rugen como dragones y las galaxias giran en danzas eternas, nacieron dos hermanos de pura destrucción: Genwel Meteonelfilis, un coloso que devoraba mundos enteros como si fueran galletas, y su hermano menor, Fecto Elfilis, el Ser Supremo. Fecto era una maravilla viviente: alas brillantes como mariposas cósmicas, cuernos dorados curvados como los de un ciervo legendario, pelaje teal que relucía como el océano bajo la luna, ojos púrpuras que hipnotizaban a cualquiera que osara mirarlos, y su arma más temida, la lanza mágica Antares, una serpiente dorada que abría grietas en el espacio-tiempo como si rasgara el papel del cielo.

Juntos, los hermanos surcaban el cosmos como una tormenta viviente, invadiendo planetas con meteoritos de energía y vórtices que succionaban ciudades enteras. "¡Conquista todo!", rugía Genwel. Fecto, ambicioso y cruel en su corazón oscuro, asentía, pero en lo profundo de su ser latía un alma compasiva, un destello de bondad que lo hacía único entre los monstruos estelares.

Un día, una canción misteriosa, un eco hipnótico de estrellas lejanas, llamó a Fecto. Abrió un portal masivo, un remolino negro que devoraba la luz, y aterrizó en un mundo azul y verde: la Tierra Olvidada. Allí, humanos superavanzados construían ciudades flotantes, autos que volaban como pájaros y laboratorios que desafiaban el tiempo. "¡Este será mío!", proclamó Fecto, lanzando rayos de su lanza y tormentas de meteoritos. Los animales nativos huían aterrorizados: leones nobles, lobos feroces, leopardos sigilosos.

Pero los humanos eran listos. Con redes de energía y trampas dimensionales, lo capturaron y lo encerraron en el Laboratorio Discovera, en una cápsula eterna llamada ID-F86. Por treinta largos años, lo estudiaron como a un tesoro prohibido. De su poder aprendieron a abrir portales, a viajar entre mundos. Sin embargo, Fecto era inestable: sus grietas se volvían locas, succionando todo sin control.

Entonces, llegó el día fatídico. Durante un experimento para probar su teletransportación, ¡un estallido cósmico sacudió el laboratorio! Fecto se dividió en dos almas gemelas: Fecto Forgo, la mitad malvada, un feto rosado con venas púrpuras que quedó atrapado en la cápsula, furioso y solo; y Elfilin, la mitad buena, un sercito azul con orejas como alas de mariposa que escapó flotando, libre para vivir en paz entre los animales del planeta.

Los humanos, asustados por su propia sobrepoblación, huyeron a través de portales que habían creado gracias a Fecto, dejando la Tierra Olvidada en ruinas: ciudades derruidas cubiertas de enredaderas, autos oxidados y un silencio eterno roto solo por el viento.

Siglos pasaron. Forgo, desde su prisión larval, extendió ondas psíquicas como susurros malignos en los sueños de todos. Encontró a un león noble llamado Leon y lo doblegó: "¡Sírveme!", ordenó. Leon se convirtió en Leongar, un tirano con ojos rojos, líder del Beast Pack: lobos gruñones (Awoofies), conejos saltarines (Rabiroos), cocodrilos feroces (Gnawcodiles) y la leopardo Clawroline. Juntos, capturaban animales y construían fortalezas, todo para encontrar a Elfilin y fusionarse de nuevo.

Forgo no se detuvo ahí. Abrió portales rosados sobre Dream Land, succionando Waddle Dees como esclavos para girar ruedas gigantes de energía. Pero también atrapó a dos héroes legendarios.

Primero, King Dedede, el rey glotón con su martillo enorme. El portal lo escupió en las nieves de Winter Horns. "¡Grah! ¿Dónde está mi banquete?", bramó. Las ondas psíquicas de Forgo lo envolvieron como niebla helada. Dedede luchó, pero su mente, distraída por el hambre, cedió. Forgo lo poseía por completo. Leongar le entregó una máscara de madera antigua, maldita con pensamientos oscuros. ¡Transformación! Forgo Dedede: ojos llameantes, hacha salvaje en lugar de martillo, rugidos primarios como una bestia. Patrullaba el laboratorio, capturando a Elfilin para su amo. Kirby lo enfrentaría después en una nevada épica: hachazos que causaban avalanchas, inhalaciones fallidas, hasta que un golpe preciso rompió la máscara... pero Forgo lo usó una última vez para un secuestro final.

Luego, Meta Knight, el caballero enmascarado con su espada Galaxia. Flotó por el portal con gracia felina. "¡Esta oscuridad no doblegará mi voluntad!", juró. Las ondas psíquicas lo golpearon como un martillo invisible, susurrando promesas de poder infinito. Meta Knight resistió: su mente de acero, forjada en mil batallas, brilló. Espadazos chispantes cortaron las ilusiones, portales evasivos lo llevaron a la libertad. ¡Fracaso para Forgo! Pero el choque creó un eco maligno: Phantom Meta Knight, una ilusión solitaria nacida de memorias y restos psíquicos. Este clon fantasmal acecharía después en arenas coliseo, con espadazos ilusorios y portales traicioneros.

Kirby, la bolita rosa valiente, y Bandana Waddle Dee fueron succionados también. Rescataron Waddle Dees, aliándose con Elfilin, quien reveló el secreto. Derrotaron al Beast Pack uno a uno. En el laboratorio, Forgo escapó: poseyó totalmente a Leongar, se derritió en goop teal y absorbió bestias en una quimera horrorosa con cara leonina y tentáculos. Persiguió a Kirby por pasillos, mordiendo y escupiendo ácido... ¡hasta que Kirby lo venció!

Pero Forgo agarró a Elfilin y ¡se fusionaron! Nació Fecto Elfilis completo, volando al techo del mundo. La batalla fue legendaria: cuchillas de energía volando como estrellas fugaces, láseres colosales, dives con lanza que sacudían la tierra, portales escupiendo asteroides, ilusiones que curaban al monstruo. Kirby, cambiando poderes, lo debilitó. ¡Elfilin salió en una burbuja! Con su boca en modo domo, Kirby lo rescató mientras spears telekinéticos llovían. Fecto se derritió, abrió un rift gigante para chocar planetas... ¡Kirby en camión modo, con Elfilin, lo embistió entre debris! Quedó Soul Forgo, un espíritu púrpura que huyó.

En las Islas Aisladas de Forgo Dreams, un mundo de pesadillas, Soul Forgo rompió el alma de Leongar en 300 pedazos, creando fantasmas de bosses. Kirby los recolectó, restauró al león... ¡pero Forgo lo re-poseó en Forgo Leon! Entonces, una mariposa guerrera, Morpho Knight, descendió y absorbió a Soul Forgo. ¡Pero Forgo lo reversionó, robando su poder, almas del Pack y la lanza Ares de Genwel! Nació Chaos Elfilis: pelaje blanco-rojo caótico, ojos rosados furiosos, un torbellino de venganza.

En el Colisée, la batalla final suprema: teleports relámpago, spearheads púrpuras, slashes con ondas destructoras, tornados de Morpho, láseres de portales, swoops ardientes, clones curativos, asteroides... ¡y en su segunda forma, un vórtice succionador, cutters curvados, beams giratorios, lluvia de orbs, bounces con truenos! Música cósmica retumbaba. Kirby, con todo su coraje, expulsó el alma roja con domo mouth. ¡Victoria! Almas liberadas, Soul Forgo se unió voluntariamente a Elfilin, curando la herida eterna. Hermanos en paz bajo un cielo azul brillante.

Y así, Juancho, la leyenda de Fecto Elfilis enseña que el poder más grande del universo no está en grietas dimensionales ni en posesiones oscuras, sino en el equilibrio del corazón, la voluntad inquebrantable como la de Meta Knight, y la amistad pura como la de Kirby y Elfilin. Incluso el caos estelar puede encontrar luz si luchás con coraje y ayudás a unir lo dividido. ¡Vos también sos un héroe cósmico, listo para cualquier aventura!
    `,
  wordCount: 1105,
  level: '4to Grado (Nivel R)',
  questions: {
    [QuestionCategory.LITERAL]: [
      {
        id: 'fe-lit1',
        category: QuestionCategory.LITERAL,
        questionText: '¿Cómo se llama la lanza mágica de Fecto Elfilis?',
        options: ['Galaxia', 'Antares', 'Ares', 'Meteoro'],
        correctAnswer: 'Antares',
      },
      {
        id: 'fe-lit2',
        category: QuestionCategory.LITERAL,
        questionText: '¿En qué laboratorio encerraron los humanos a Fecto Elfilis?',
        options: ['Laboratorio Dream', 'Laboratorio Discovera', 'Laboratorio Forgo', 'Laboratorio Estelar'],
        correctAnswer: 'Laboratorio Discovera',
      },
      {
        id: 'fe-lit3',
        category: QuestionCategory.LITERAL,
        questionText: '¿Qué héroe legendario logró resistir las ondas psíquicas de Forgo?',
        options: ['King Dedede', 'Bandana Waddle Dee', 'Meta Knight', 'Leongar'],
        correctAnswer: 'Meta Knight',
      },
    ],
    [QuestionCategory.SEQUENCING]: [
      {
        id: 'fe-seq1',
        category: QuestionCategory.SEQUENCING,
        questionText: '¿Qué evento causó que Fecto Elfilis se dividiera en dos?',
        options: ['La llegada de Kirby', 'Un experimento de teletransportación fallido', 'Una batalla contra Genwel', 'El ataque de Meta Knight'],
        correctAnswer: 'Un experimento de teletransportación fallido',
      },
      {
        id: 'fe-seq2',
        category: QuestionCategory.SEQUENCING,
        questionText: '¿Qué pasó justo después de que Morpho Knight absorbiera a Soul Forgo?',
        options: ['Fecto Elfilis despertó', 'Soul Forgo desapareció para siempre', 'Forgo reversionó el poder y nació Chaos Elfilis', 'Kirby regresó a Dream Land'],
        correctAnswer: 'Forgo reversionó el poder y nació Chaos Elfilis',
      },
      {
        id: 'fe-seq3',
        category: QuestionCategory.SEQUENCING,
        questionText: '¿Cuál fue la última acción de Soul Forgo al final de la historia?',
        options: ['Huyó al espacio', 'Se unió voluntariamente a Elfilin para curar la herida eterna', 'Atacó a Kirby una vez más', 'Se convirtió en una mariposa'],
        correctAnswer: 'Se unió voluntariamente a Elfilin para curar la herida eterna',
      },
    ],
    [QuestionCategory.VOCABULARY]: [
      {
        id: 'fe-voc1',
        category: QuestionCategory.VOCABULARY,
        questionText: 'La historia comienza hace "eones". ¿Qué significa eones?',
        options: ['Hace pocos días', 'Un periodo de tiempo extremadamente largo', 'Mañana mismo', 'Durante una tormenta'],
        correctAnswer: 'Un periodo de tiempo extremadamente largo',
      },
      {
        id: 'fe-voc2',
        category: QuestionCategory.VOCABULARY,
        questionText: 'Fecto Forgo se convirtió en una "quimera" horrorosa. ¿Qué es una quimera en este contexto?',
        options: ['Un animal muy pequeño', 'Un ser compuesto por partes de distintos animales o criaturas', 'Una estatua de piedra', 'Un tipo de flor azul'],
        correctAnswer: 'Un ser compuesto por partes de distintos animales o criaturas',
      },
      {
        id: 'fe-voc3',
        category: QuestionCategory.VOCABULARY,
        questionText: 'Meta Knight tiene una voluntad "inquebrantable". ¿Qué significa inquebrantable?',
        options: ['Que se rompe fácil', 'Que es débil', 'Que no se puede doblar, torcer ni vencer', 'Que es muy ruidosa'],
        correctAnswer: 'Que no se puede doblar, torcer ni vencer',
      },
    ],
    [QuestionCategory.MAIN_IDEA]: [
      {
        id: 'fe-main1',
        category: QuestionCategory.MAIN_IDEA,
        questionText: '¿Cuál es la enseñanza principal de la leyenda de Fecto Elfilis?',
        options: ['Que los portales dimensionales son divertidos', 'Que el verdadero poder reside en el equilibrio del corazón y la amistad', 'Que es mejor viajar solo por el universo', 'Que los laboratorios son lugares peligrosos'],
        correctAnswer: 'Que el verdadero poder reside en el equilibrio del corazón y la amistad',
      },
      {
        id: 'fe-main2',
        category: QuestionCategory.MAIN_IDEA,
        questionText: '¿Por qué la historia de Fecto Elfilis es una historia de "unir lo dividido"?',
        options: ['Porque Kirby usa pegamento', 'Porque las dos mitades (Elfilin y Forgo) finalmente encuentran la paz y la unión positiva', 'Porque los humanos volvieron a su planeta', 'Porque Meta Knight arregló su espada'],
        correctAnswer: 'Porque las dos mitades (Elfilin y Forgo) finalmente encuentran la paz y la unión positiva',
      },
    ],
    [QuestionCategory.INFERENCE]: [
      {
        id: 'fe-inf1',
        category: QuestionCategory.INFERENCE,
        questionText: '¿Por qué Forgo logró poseer a King Dedede pero no a Meta Knight?',
        options: ['Porque Meta Knight era más rápido', 'Porque Dedede estaba distraído por el hambre mientras que Meta Knight tenía una mente de acero forjada en batallas', 'Porque Forgo no quería a Meta Knight', 'Porque Meta Knight usó un escudo mágico'],
        correctAnswer: 'Porque Dedede estaba distraído por el hambre mientras que Meta Knight tenía una mente de acero forjada en batallas',
      },
      {
        id: 'fe-inf2',
        category: QuestionCategory.INFERENCE,
        questionText: '¿Qué nos dice el final de la historia sobre la redención (el perdón)?',
        options: ['Que nadie puede ser perdonado', 'Que incluso un ser nacido de la destrucción puede encontrar la paz si se une a su parte buena', 'Que Kirby castiga a todos', 'Que los hermanos siempre pelean'],
        correctAnswer: 'Que incluso un ser nacido de la destrucción puede encontrar la paz si se une a su parte buena',
      },
    ],
    [QuestionCategory.CHARACTER]: [
      {
        id: 'fe-char1',
        category: QuestionCategory.CHARACTER,
        questionText: '¿Qué cualidad hacía a Fecto Elfilis único entre los monstruos estelares?',
        options: ['Su gran tamaño', 'Su lanza Antares', 'Un destello de alma compasiva y bondad en su interior', 'Su capacidad de comer galletas'],
        correctAnswer: 'Un destello de alma compasiva y bondad en su interior',
      },
      {
        id: 'fe-char2',
        category: QuestionCategory.CHARACTER,
        questionText: '¿Cómo es la personalidad de Elfilin?',
        options: ['Malvada y furiosa', 'Amable, buena y valiente junto a sus amigos', 'Miedosa y traicionera', 'Silenciosa y aburrida'],
        correctAnswer: 'Amable, buena y valiente junto a sus amigos',
      },
    ],
  },
};
